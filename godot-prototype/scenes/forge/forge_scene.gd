...
func _build_god_panel() -> void:
	# 神祇设置面板统一为公共组件(scenes/common/god_settings_panel.gd),
	# 与开始界面 / 铁匠铺保持一致的字段、保存与连接检测行为。
	var GodSettingsPanel = preload("res://scenes/common/god_settings_panel.gd")
	ui.god_panel = GodSettingsPanel.new()
	ui.god_panel.position = Vector2(390, 100)
	add_child(ui.god_panel)
