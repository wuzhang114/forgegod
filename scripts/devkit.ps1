...
## 注意: Godot 控制台输出走 stderr,必须 2>&1 合并;
##       且脚本内外部命令必须经管道(| Out-Host)才能被宿主捕获。
param(
	[string]$Action = "test",
...
switch ($Action.ToLower()) {
	"test" {
		...
		& $Godot --path $Project --headless -s tests/run_headless.gd 2>&1 | Out-Host
	}
	"run" {
		Write-Host "== 启动 $Scene ==" -ForegroundColor Cyan
		& $Godot --path $Project $Scene 2>&1 | Out-Host
	}
	"import" {
		Write-Host "== 资源导入(新贴图/音频后) ==" -ForegroundColor Cyan
		& $Godot --path $Project --headless --import 2>&1 | Out-Host
	}
	"smoke" {
		Write-Host "== 冒烟: $Scene (150 帧) ==" -ForegroundColor Cyan
		& $Godot --path $Project $Scene --quit-after 150 2>&1 | Out-Host
	}
	"diag" {
		Write-Host "== 诊断: $Diag ==" -ForegroundColor Cyan
		& $Godot --path $Project --quit-after 900 -s $Diag 2>&1 | Out-Host
	}
	"quit" {
